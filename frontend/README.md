...
- `src/app`: Global configuration, routing, and provider setup.
- `src/components`: UI components organized by layout, sections, and atomic UI elements.
- `src/features`: Domain-specific components and logic (e.g., Blog, Services, Case Studies).
- `src/pages`: Main entry points for each application route.
- `src/styles`: Global styles, Tailwind configuration, and theme tokens.
